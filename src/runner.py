...
def conv_runner(env, policy, num_local_steps, summary_writer, render,reward_f=None,shared = False):
    """
The logic of the thread runner.  In brief, it constantly keeps on running
the policy, and as long as the rollout exceeds a certain length, the thread
runner appends the policy to the queue.
"""
    ...
    length = 0
    rewards = 0
    mem_size = reward_f.mem_size
    r_obs = np.zeros(reward_f.ob_space[:-1]+(mem_size,))
    while True:
        terminal_end = False
        rollout = PartialRollout()
        for _ in range(num_local_steps):
            ...
            if reward_f is not None:
                # If there is an external reward function use that.
                r_obs[:,:,:mem_size-1] = r_obs[:,:,1:mem_size]
                r_obs[:,:,-1] = last_state[:,:,0]
                r_fetched = reward_f.reward([r_obs])
                #reward = r_fetched[0][0,0] #-r_fetched[0][0,1] #if reward is binary class.
                reward = r_fetched[0][0, action.argmax()]
                irl_rewards.append(reward)
                rollout.add(last_state, action, reward, value_, terminal, last_features)
