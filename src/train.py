...
parser.add_argument('-c', '--cfg', type=str, default=None,
                    help="Configuration file with all the options learning rates architecture reward modes etc")
...
def create_commands(session, num_workers, remotes, env_id, logdir, shell='bash', mode='tmux', visualise=False,cfg = None,record=False):
...
def run():
    ...
    cmds, notes = create_commands("a3c", args.num_workers, args.remotes, args.env_id, args.log_dir, mode=args.mode,
                                  visualise=args.visualise,record=args.record,cfg=args.cfg)
